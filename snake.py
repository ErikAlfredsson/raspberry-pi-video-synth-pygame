...
class Snake(App):
    ...
    def setup(self, screen, data):
        ...
        self.draw_rect = pygame.Rect(
            ...
            0,
            int(screen.get_width() // 2 + self.size * 2),
    ...
    def draw(self, screen, data):
        ...
        for i in range(start, end):
            ...
            xpos = int(
                screen.get_width() // 2 + self.size * math.sin(i * 0.02 + current_time)
            )
            ...
            color = (
                int(
                    ...
                    + 127
                    * math.sin(
                        i * (0.01 + 0.02 * self.filtered_magnitude) + current_time
                    )
                ),
                int(127 + 127 * math.sin(i * 0.011 + current_time)),
                int(127 + 127 * math.sin(i * 0.012 + current_time)),
            )
            ...
            radius = int(
                self.size
                + (self.size * 0.5 + (1 - normalized_index) * self.size * 0.5)
                * data["low_pass"]
            )
